...
type GetGroupCurrenciesRow struct {
	ID         int64
	Code       string
	Name       string
	Symbol     string
...
func (q *Queries) GetGroupCurrencies(ctx context.Context, groupID int64) ([]GetGroupCurrenciesRow, error) {
	rows, err := q.db.Query(ctx, getGroupCurrencies, groupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetGroupCurrenciesRow
	for rows.Next() {
		var i GetGroupCurrenciesRow
		if err := rows.Scan(
			&i.ID,
			&i.Code,
			&i.Name,
